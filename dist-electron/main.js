...
import require$$1$3, { app, shell, ipcMain, BrowserWindow } from "electron";
import require$$1$4, { spawn } from "child_process";
import { fileURLToPath } from "node:url";
import path$n from "node:path";
import * as fs$k from "fs";
import fs__default from "fs";
import require$$1 from "path";
import os$2 from "os";
import * as crypto$1 from "crypto";
import crypto__default, { createHash, randomUUID } from "crypto";
...
function _vaultPath(options) {
  ...
  if (options && options.path && options.path.length > 0) {
    ...
    possibleVaultPath = path$m.resolve(process.cwd(), ".env.vault");
  }
...
function _resolveHome(envPath) {
  return envPath[0] === "~" ? path$m.join(os$1.homedir(), envPath.slice(1)) : envPath;
}
...
function configDotenv(options) {
  const dotenvPath = path$m.resolve(process.cwd(), ".env");
  let encoding = "utf8";
  let processEnv = process.env;
  if (options && options.processEnv != null) {
    processEnv = options.processEnv;
  ...
  if (debug2 || !quiet) {
    ...
    for (const filePath of optionPaths) {
      try {
        const relative = path$m.relative(process.cwd(), filePath);
...
function initDb() {
  if (!db$1) {
    try {
      const dbPath = require$$1.join(app.getPath("userData"), "database.sqlite");
      const isDev2 = process.env.NODE_ENV !== "production";
      db$1 = new Database(dbPath, isDev2 ? { verbose: console.log } : {});
      ...
      `);
      try {
        const tableInfo = db$1.pragma("table_info(users)");
        const hasVerifyToken = tableInfo.some((col) => col.name === "master_verify_token");
        if (!hasVerifyToken) {
          db$1.exec("ALTER TABLE users ADD COLUMN master_verify_token TEXT");
...
function generateSalt() {
  return crypto__default.randomBytes(16).toString("hex");
}
function hashPassword(password, salt) {
  return crypto__default.pbkdf2Sync(password, salt, 1e5, 64, "sha512").toString("hex");
}
...
function deriveEncryptionKey(masterPassword, salt) {
  return crypto__default.scryptSync(masterPassword, salt, 32, {
    N: 131072,
    // 2^17 - strong memory hardness
    r: 8,
    // Block size
    p: 1,
    // Parallelization
...
function encryptPassword(plaintext, encryptionKey) {
  try {
    ...
    const cipher = crypto__default.createCipheriv("aes-256-gcm", encryptionKey, iv);
...
function decryptPassword(encryptedData, encryptionKey) {
  try {
    ...
    const decipher = crypto__default.createDecipheriv("aes-256-gcm", encryptionKey, iv);
    decipher.setAuthTag(authTag);
    let decrypted = decipher.update(encryptedHex, "hex", "utf8");
    decrypted += decipher.final("utf8");
    return decrypted;
  } catch (error2) {
    console.error("Decryption error:", error2.message);
    if (error2.message.includes("Unsupported state or unable to authenticate data")) {
...
const crypto_1$4 = crypto__default;
...
const crypto_1$3 = crypto__default;
...
const crypto_1$2 = crypto__default;
...
const crypto_1$1 = crypto__default;
const os_1 = os$2;
...
function initMailDatabase() {
  ...
  const dbPath = require$$1.join(userDataPath, "mail.db");
...
function runMigrations() {
  ...
  const hasSecurityColumns = tableInfo.some((col) => col.name === "is_suspicious");
  if (!hasSecurityColumns) {
    ...
    try {
      ...
      console.error("[Mail DB] Migration error:", error2);
      throw error2;
    }
  }
}
...
function getAllAccounts() {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM mail_accounts ORDER BY created_at ASC").all();
}
function getAccountById(accountId) {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM mail_accounts WHERE id = ?").get(accountId);
}
...
function deleteAccount(accountId) {
  const db2 = getMailDatabase();
  return db2.prepare("DELETE FROM mail_accounts WHERE id = ?").run(accountId);
}
function updateAccountSyncStatus(accountId, lastSyncTime) {
  const db2 = getMailDatabase();
  return db2.prepare(`
...
function getFoldersByAccount(accountId) {
  const db2 = getMailDatabase();
  const folders = db2.prepare("SELECT * FROM mail_folders WHERE account_id = ?").all(accountId);
...
function getEmailsByFolder(accountId, folder, limit = 50, offset = 0) {
  const db2 = getMailDatabase();
  return db2.prepare(`
    SELECT * FROM emails
...
function getEmailById(emailId) {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM emails WHERE id = ?").get(emailId);
}
...
function markEmailAsRead(emailId, isRead) {
  const db2 = getMailDatabase();
  return db2.prepare(`
    UPDATE emails
...
function deleteEmail(emailId) {
  const db2 = getMailDatabase();
  return db2.prepare("DELETE FROM emails WHERE id = ?").run(emailId);
}
function updateEmailBody(emailId, textBody, htmlBody, isSuspicious, securityScore, securityRating, securityIssues, securitySummary) {
  const db2 = getMailDatabase();
  return db2.prepare(`
    UPDATE emails
...
function getAttachmentsByEmail(emailId) {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM email_attachments WHERE email_id = ?").all(emailId);
}
...
function getSignaturesByAccount(accountId) {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM email_signatures WHERE account_id = ? ORDER BY is_default DESC, created_at DESC").all(accountId);
}
function getSignatureById(signatureId) {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM email_signatures WHERE id = ?").get(signatureId);
}
function getDefaultSignature(accountId) {
  const db2 = getMailDatabase();
  return db2.prepare("SELECT * FROM email_signatures WHERE account_id = ? AND is_default = 1 LIMIT 1").get(accountId);
}
function addSignature(signature) {
  const db2 = getMailDatabase();
  if (signature.isDefault) {
    db2.prepare("UPDATE email_signatures SET is_default = 0 WHERE account_id = ?").run(signature.accountId);
  }
  const stmt = db2.prepare(`
...
function updateSignature(signatureId, name, htmlContent, isDefault) {
  const db2 = getMailDatabase();
  ...
  if (isDefault) {
    db2.prepare("UPDATE email_signatures SET is_default = 0 WHERE account_id = ?").run(signature.account_id);
  }
  return db2.prepare(`
...
function deleteSignature(signatureId) {
  const db2 = getMailDatabase();
  return db2.prepare("DELETE FROM email_signatures WHERE id = ?").run(signatureId);
}
const dnsResolve$1 = promisify(dns.resolve);
const dnsReverse$1 = promisify(dns.reverse);
class EmailSecurityScanner {
  constructor() {
    __publicField(this, "issues", []);
    __publicField(this, "score", 10);
  }
   ...
   */
  checkAuthenticationHeaders(headers) {
   ...
   */
  checkDomainMismatch(email) {
    var _a;
    const fromAddress = (_a = email.from[0]) == null ? void 0 : _a.address;
   ...
   */
  checkDisplayNameSpoofing(email) {
   ...
   */
  async checkIPReputation(headers) {
    try {
   ...
   */
  async checkReverseDNS(headers) {
    try {
      const ip = this.extractIPFromReceived(headers.received);
      if (!ip) return;
      const hostnames = await dnsReverse$1(ip);
  ...
  extractIPFromReceived(received) {
  ...
  async checkBlacklist(ip, blacklist) {
    try {
      const reversed = ip.split(".").reverse().join(".");
      const query = `${reversed}.${blacklist}`;
      await dnsResolve$1(query, "A");
      return true;
    } catch {
      return false;
    }
...
const emailSecurityScanner = new EmailSecurityScanner();
const connectionPool = /* @__PURE__ */ new Map();
async function connectIMAP(accountId, config2) {
  const existing = connectionPool.get(accountId);
  if (existing && existing.usable) {
    return existing;
  }
  console.log(`[IMAP] Connecting to ${config2.host}:${config2.port} for account ${accountId}`);
  const client = new ImapFlow({
    ...
    greetingTimeout: 3e4,
    // 30 seconds for server greeting
    connectionTimeout: 3e4,
    // 30 seconds for connection establishment
    // TLS options
    tls: {
      rejectUnauthorized: false,
      // Allow self-signed certificates (change to true in production)
      minVersion: "TLSv1.2"
    }
  });
  try {
    await client.connect();
    console.log(`[IMAP] Connected successfully to ${config2.host}`);
    ...
    console.error(`[IMAP] Connection failed:`, error2);
    throw new Error(`Failed to connect to IMAP server: ${error2.message}`);
  }
}
async function disconnectIMAP(accountId) {
  ...
  if (client) {
    try {
      ...
      console.error(`[IMAP] Logout error:`, error2);
    } finally {
      connectionPool.delete(accountId);
    }
  }
}
async function testIMAPConnection(config2) {
  var _a, _b, _c, _d;
  console.log(`[IMAP] Testing connection to ${config2.host}:${config2.port} (secure: ${config2.secure})`);
  ...
  try {
    ...
    console.error("[IMAP] Test connection failed:", error2);
    ...
    if (error2.code === "ETIMEDOUT" || ((_a = error2.message) == null ? void 0 : _a.includes("timeout"))) {
      errorMessage = `Connection timeout to ${config2.host}:${config2.port}. Check firewall, network, or server availability.`;
    } else if (error2.code === "ECONNREFUSED") {
      errorMessage = `Connection refused by ${config2.host}:${config2.port}. Check if server is running and port is correct.`;
    } else if (error2.code === "ENOTFOUND") {
      errorMessage = `Server ${config2.host} not found. Check hostname spelling and DNS settings.`;
    } else if ((_b = error2.message) == null ? void 0 : _b.includes("certificate")) {
      errorMessage = `SSL/TLS certificate error. Server may use self-signed certificate.`;
    } else if (((_c = error2.message) == null ? void 0 : _c.includes("authentication")) || ((_d = error2.message) == null ? void 0 : _d.includes("auth"))) {
      errorMessage = `Authentication failed. Check username and password. For 2FA accounts, use an app-specific password.`;
    }
    const enhancedError = new Error(errorMessage);
    enhancedError.originalError = error2;
    throw enhancedError;
  }
}
async function syncFolders(accountId, client) {
  console.log(`[IMAP] Syncing folders for account ${accountId}`);
  try {
    const list = await client.list();
    const folders = [];
    for (const item of list) {
      const folderId = crypto$1.randomUUID();
      let totalMessages = 0;
    ...
    console.error("[IMAP] Folder sync error:", error2);
    throw error2;
  }
}
async function fetchEmails(accountId, client, folderPath, limit = 50) {
  var _a, _b, _c, _d, _e;
  ...
  try {
    ...
    console.log(`[IMAP] Fetching messages ${range2}`);
    const emails = [];
    for await (const message of client.fetch(range2, {
      envelope: true,
      bodyStructure: true,
      ...
      uid: true,
      source: true
      // Get full source for body parsing
    })) {
      try {
        const emailId = crypto$1.randomUUID();
        let from = [];
        let to = [];
        let cc = [];
        let bcc = [];
        ...
        if (message.bodyParts && message.bodyParts.get("HEADER")) {
          try {
            ...
            from = ((_a = parsed.from) == null ? void 0 : _a.value) || [];
            to = ((_b = parsed.to) == null ? void 0 : _b.value) || [];
            cc = ((_c = parsed.cc) == null ? void 0 : _c.value) || [];
            bcc = ((_d = parsed.bcc) == null ? void 0 : _d.value) || [];
            replyTo = ((_e = parsed.replyTo) == null ? void 0 : _e.value) || [];
        ...
        if (from.length === 0 && message.envelope) {
          ...
          date = message.envelope.date || /* @__PURE__ */ new Date();
        ...
        if (message.bodyStructure && message.bodyStructure.childNodes) {
          hasAttachments = message.bodyStructure.childNodes.some(
            (node2) => node2.disposition === "attachment" || node2.type && !node2.type.includes("text") && !node2.type.includes("multipart")
          );
        }
        let textBody = null;
        let htmlBody = null;
        ...
        const email = {
          ...
          replyTo,
          subject,
          date: date instanceof Date ? date.toISOString() : (/* @__PURE__ */ new Date()).toISOString(),
    ...
    updateAccountSyncStatus(accountId, (/* @__PURE__ */ new Date()).toISOString());
    return emails.reverse();
  } catch (error2) {
    console.error("[IMAP] Fetch emails error:", error2);
    throw error2;
  }
}
async function fetchEmailBody(accountId, client, folderPath, uid, emailId) {
  var _a;
  ...
  try {
    ...
    for await (const message of messages) {
      ...
      if (parsed.attachments) {
        for (const att of parsed.attachments) {
          const attachmentId = crypto$1.randomUUID();
          const attachment = {
            id: attachmentId,
            emailId,
      ...
      try {
        const from = ((_a = parsed.from) == null ? void 0 : _a.value) || [];
        ...
        if (securityReport.isSuspicious) {
          ...
          console.log(`  From: ${from.map((f) => f.address).join(", ")}`);
    ...
    console.error("[IMAP] Fetch email body error:", error2);
    throw error2;
  }
}
async function markAsRead(client, folderPath, uid, isRead) {
  try {
    await client.mailboxOpen(folderPath);
    if (isRead) {
    ...
    console.error("[IMAP] Mark as read error:", error2);
    throw error2;
  }
}
async function deleteMessage(client, folderPath, uid, permanent = false) {
  try {
    await client.mailboxOpen(folderPath);
    if (permanent) {
    ...
    console.error("[IMAP] Delete message error:", error2);
    throw error2;
  }
}
async function appendMessage(client, folder, rawMessage, flags = ["\\Seen"]) {
  try {
    ...
    console.error(`[IMAP] Append message to ${folder} error:`, error2);
    throw error2;
  }
}
function getIMAPConnection(accountId) {
  return connectionPool.get(accountId) || null;
}
const idleListeners = /* @__PURE__ */ new Map();
async function startIdle(accountId, folder = "INBOX", onNewMail) {
  try {
    const client = connectionPool.get(accountId);
    ...
    console.error("[IMAP IDLE] Error starting IDLE:", error2);
  }
}
async function stopIdle(accountId) {
  try {
    const listener = idleListeners.get(accountId);
    if (!listener) return;
    ...
    console.error("[IMAP IDLE] Error stopping IDLE:", error2);
  }
}
const transporterPool = /* @__PURE__ */ new Map();
function createSMTPTransporter(accountId, config2) {
  const existing = transporterPool.get(accountId);
  if (existing) {
    return existing;
  }
  console.log(`[SMTP] Creating transporter for ${config2.host}:${config2.port}`);
...
async function testSMTPConnection(config2) {
  var _a, _b, _c, _d;
  console.log(`[SMTP] Testing connection to ${config2.host}:${config2.port} (secure: ${config2.secure})`);
  const testTransporter = nodemailer.createTransport({
    ...
    greetingTimeout: 3e4,
    // 30 seconds for greeting
    socketTimeout: 6e4,
    // 60 seconds for socket
    // TLS options
    tls: {
      rejectUnauthorized: false,
      // Allow self-signed certificates
  ...
  try {
    ...
    console.error("[SMTP] Connection test failed:", error2);
    ...
    if (error2.code === "ETIMEDOUT" || ((_a = error2.message) == null ? void 0 : _a.includes("timeout"))) {
      ...
      errorMessage = `Connection refused by ${config2.host}:${config2.port}. Check if SMTP server is running and port is correct.`;
    } else if (error2.code === "ENOTFOUND") {
      errorMessage = `Server ${config2.host} not found. Check hostname spelling and DNS settings.`;
    } else if ((_b = error2.message) == null ? void 0 : _b.includes("certificate")) {
      errorMessage = `SSL/TLS certificate error. Server may use self-signed certificate.`;
    } else if (error2.responseCode === 535 || ((_c = error2.message) == null ? void 0 : _c.includes("authentication")) || ((_d = error2.message) == null ? void 0 : _d.includes("auth"))) {
      errorMessage = `Authentication failed. Check username and password. For 2FA accounts, use an app-specific password.`;
    }
    testTransporter.close();
    const enhancedError = new Error(errorMessage);
...
async function sendEmail(accountId, transporter, email) {
  ...
  try {
    ...
    if (email.attachments && email.attachments.length > 0) {
      mailOptions.attachments = email.attachments.map((att) => {
        if (att.path && fs$k.existsSync(att.path)) {
          return {
            filename: att.filename,
            path: att.path,
            contentType: att.contentType
    ...
    console.error("[SMTP] Send email error:", error2);
    throw new Error(`Failed to send email: ${error2.message}`);
  }
}
async function sendEmailWithRetry(accountId, transporter, email, maxRetries = 3) {
  let lastError = null;
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error2;
      console.error(`[SMTP] Attempt ${attempt} failed:`, error2.message);
  ...
  throw new Error(`Failed to send email after ${maxRetries} attempts: ${lastError == null ? void 0 : lastError.message}`);
}
async function openAttachment(filePath) {
  ...
  try {
    ...
    console.error("[Attachments] Open error:", error2);
    throw error2;
  }
}
class MailMemoryCache {
  constructor() {
    ...
    __publicField(this, "emailIndex", /* @__PURE__ */ new Map());
    ...
    __publicField(this, "MAX_EMAILS_PER_FOLDER", 1e3);
  }
  /**
   * Get emails from cache (instant, no DB read)
   */
  getEmails(accountId, folder) {
   ...
   */
  updateCache(accountId, folder, emails) {
    if (!this.cache.has(accountId)) {
      this.cache.set(accountId, /* @__PURE__ */ new Map());
    }
    const accountCache = this.cache.get(accountId);
    ...
    const cachedEmails = emails.map((email) => {
      ...
      return {
        ...
        isRead: preserveReadStatus ? true : email.isRead !== void 0 ? email.isRead : email.is_read === 1,
        isFlagged: email.isFlagged !== void 0 ? email.isFlagged : email.is_flagged === 1,
        size: email.size || 0,
        isSuspicious: email.isSuspicious !== void 0 ? email.isSuspicious : email.is_suspicious === 1,
  ...
  upsertEmail(email) {
    ...
    const cachedEmail = {
      ...
      htmlBody: email.htmlBody || email.html_body || null,
      hasAttachments: email.hasAttachments || email.has_attachments || false,
      isRead: email.isRead !== void 0 ? email.isRead : email.is_read === 1,
      isFlagged: email.isFlagged !== void 0 ? email.isFlagged : email.is_flagged === 1,
      size: email.size || 0,
      isSuspicious: email.isSuspicious !== void 0 ? email.isSuspicious : email.is_suspicious === 1,
      securityScore: email.securityScore || email.security_score || null,
    ...
    if (existingIndex >= 0) {
      ...
      folderEmails.unshift(cachedEmail);
    }
   ...
   */
  isBodyLoaded(emailId) {
    const email = this.emailIndex.get(emailId);
    return (email == null ? void 0 : email.bodyLoaded) || false;
  }
  /**
   * Update email body in cache
   */
  updateEmailBody(emailId, textBody, htmlBody) {
    const email = this.emailIndex.get(emailId);
    if (!email) return false;
...
const mailCache = new MailMemoryCache();
const dnsResolve = promisify(dns.resolve);
const dnsResolveMx = promisify(dns.resolveMx);
const dnsResolveTxt = promisify(dns.resolveTxt);
const dnsReverse = promisify(dns.reverse);
class EmailReputationChecker {
  constructor() {
    // Major DNS Blacklists (RBLs)
    __publicField(this, "RBL_LISTS", [
   ...
   */
  async checkEmailReputation(email, sendingIP) {
    ...
    if (!ip) {
      try {
        ...
        console.error("[Reputation] Could not resolve sending IP:", error2);
      }
    }
  ...
  async checkDomainReputation(domain) {
    ...
    const blacklistStatus = this.getResult(checks[4], "Blacklist check failed", []);
  ...
  async checkIPReputation(ip) {
    ...
    const blacklistStatus = this.getResult(checks[0], "IP blacklist check failed", []);
...
${issues.join("\n")}`,
  ...
  async checkMXRecords(domain) {
    try {
      ...
      const recordList = mxRecords.sort((a, b) => a.priority - b.priority).map((mx) => `${mx.exchange} (priority: ${mx.priority})`).join("\n");
      return {
        name: "MX Records",
        status: "good",
  ...
  async checkSSLCertificate(domain) {
    return new Promise((resolve) => {
      ...
      const req = https.request(options, (res) => {
        var _a;
        const cert = (_a = res.socket) == null ? void 0 : _a.getPeerCertificate();
        if (cert && cert.subject) {
          const validTo = new Date(cert.valid_to);
          const now = /* @__PURE__ */ new Date();
          const daysUntilExpiry = Math.floor((validTo.getTime() - now.getTime()) / (1e3 * 60 * 60 * 24));
...
Expires: ${validTo.toLocaleDateString()} (${daysUntilExpiry} days)`,
  ...
  async checkSingleBlacklist(query, blacklist, name, weight) {
    try {
  ...
  getResult(result, errorMessage, defaultValue) {
    if (result.status === "fulfilled") {
      return result.value;
    } else {
  ...
  calculateBlacklistScore(blacklists) {
    ...
    if (goodCount === blacklists.length) return 100;
    return 50;
  }
  ...
  generateSummary(domain, ip, deliverabilityScore) {
...
const reputationChecker = new EmailReputationChecker();
...
async function getAccountPassword(accountId) {
  ...
  const activeSession2 = getActiveSession == null ? void 0 : getActiveSession();
  if (!activeSession2 || !activeSession2.masterPassword) {
    console.log("[Mail IPC] No active session with master password");
    return null;
  }
  const db2 = getDb == null ? void 0 : getDb();
  if (!db2) {
    console.log("[Mail IPC] Database not available");
    return null;
  }
  try {
    const credential = db2.prepare("SELECT * FROM credentials WHERE id = ?").get(account.credential_id);
    if (!credential) {
      console.log("[Mail IPC] Credential not found in vault");
      return null;
    }
    const encryptionKey = deriveEncryptionKey(activeSession2.masterPassword, activeSession2.salt);
    ...
    console.error("[Mail IPC] Error decrypting password:", error2);
    return null;
  }
}
async function syncNewEmails(accountId, folder, limit = 50) {
  try {
    console.log(`[Mail IPC] Background sync: Checking for new emails in ${folder}...`);
    const account = getAccountById(accountId);
    if (!account) return;
    const activeSession2 = getActiveSession == null ? void 0 : getActiveSession();
    if (!activeSession2 || !activeSession2.masterPassword) return;
    const db2 = getDb == null ? void 0 : getDb();
    if (!db2) return;
    const credential = db2.prepare("SELECT * FROM credentials WHERE id = ?").get(account.credential_id);
    ...
    console.error("[Mail IPC] Background sync error:", error2);
  }
}
function buildRawMessage(email) {
  const lines = [];
  ...
  lines.push(`Message-ID: <${crypto$1.randomUUID()}@sent-mail>`);
  if (email.inReplyTo) {
...
function initMailIPC(sessionGetter, dbGetter) {
  ...
  ipcMain.handle("mail:getAccounts", async () => {
    try {
      ...
      console.error("[Mail IPC] Get accounts error:", error2);
  ...
  ipcMain.handle("mail:addAccount", async (event, { account, password }) => {
    try {
      ...
      const accountId = crypto$1.randomUUID();
      const imapConfig = {
      ...
      let esMailsCategory = db2.prepare(`
        SELECT id FROM categories WHERE user_id = ? AND name = 'EsMails Crds'
      `).get(activeSession2.userId);
      if (!esMailsCategory) {
        const categoryResult = db2.prepare(`
          INSERT INTO categories (user_id, name, color)
          VALUES (?, 'EsMails Crds', '#10b981')
        `).run(activeSession2.userId);
        esMailsCategory = { id: categoryResult.lastInsertRowid };
      ...
      console.error("[Mail IPC] Add account error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:deleteAccount", async (event, { accountId }) => {
    try {
      ...
      console.error("[Mail IPC] Delete account error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:testConnection", async (event, { account, password }) => {
    try {
      ...
      console.error("[Mail IPC] Test connection error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:getFolders", async (event, { accountId }) => {
    try {
      ...
      console.error("[Mail IPC] Get folders error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:syncFolders", async (event, { accountId, password }) => {
    try {
      ...
      console.error("[Mail IPC] Sync folders error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:getEmails", async (event, { accountId, folder, password, limit = 50 }) => {
    try {
      ...
      if (hasCache) {
        ...
        const emails2 = cachedEmails.map((email) => {
        ...
        mailCache.updateCache(accountId, folder, emails2);
        setImmediate(async () => {
          try {
            await syncNewEmails(accountId, folder, limit);
          } catch (error2) {
        ...
        return { success: true, data: emails2, cached: true, source: "database", syncing: true };
      }
      const account = getAccountById(accountId);
      if (!account) {
        throw new Error("Account not found");
      ...
      console.error("[Mail IPC] Get emails error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:getEmailsFresh", async (event, { accountId, folder, password, limit = 50 }) => {
    try {
      ...
      console.error("[Mail IPC] Fresh fetch error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:getEmail", async (event, { emailId }) => {
    try {
      ...
      console.error("[Mail IPC] Get email error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:fetchEmailBody", async (event, { accountId, emailId, password }) => {
    ...
    try {
      ...
      if (!accountPassword && account.credential_id) {
        const session = getActiveSession == null ? void 0 : getActiveSession();
        if (!(session == null ? void 0 : session.masterPassword) || !(session == null ? void 0 : session.salt)) {
      ...
      updateEmailBody(
        ...
        bodyData.textBody,
        bodyData.htmlBody,
        ((_a = bodyData.securityReport) == null ? void 0 : _a.isSuspicious) || false,
        ((_b = bodyData.securityReport) == null ? void 0 : _b.score) || null,
        ((_c = bodyData.securityReport) == null ? void 0 : _c.rating) || null,
        bodyData.securityReport ? JSON.stringify(bodyData.securityReport.issues) : null,
        ((_d = bodyData.securityReport) == null ? void 0 : _d.summary) || null
      );
      ...
      return {
        ...
        data: {
          ...
          isSuspicious: ((_e = bodyData.securityReport) == null ? void 0 : _e.isSuspicious) || false,
          securityScore: ((_f = bodyData.securityReport) == null ? void 0 : _f.score) || null,
          securityRating: ((_g = bodyData.securityReport) == null ? void 0 : _g.rating) || null,
          securityIssues: ((_h = bodyData.securityReport) == null ? void 0 : _h.issues) || [],
          securitySummary: ((_i = bodyData.securityReport) == null ? void 0 : _i.summary) || null
        },
        cached: false
      };
    } catch (error2) {
      console.error("[Mail IPC] Fetch email body error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:sendEmail", async (event, { accountId, draft, password }) => {
    ...
    try {
      ...
      const emailToSend = {
        from: account.email,
        to: draft.to.map((addr) => addr.address || addr),
        cc: (_a = draft.cc) == null ? void 0 : _a.map((addr) => addr.address || addr),
        bcc: (_b = draft.bcc) == null ? void 0 : _b.map((addr) => addr.address || addr),
      ...
      try {
        ...
        console.error("[Mail IPC] Error saving sent message:", error2);
      }
      return { success: true, data: result };
    } catch (error2) {
      console.error("[Mail IPC] Send email error:", error2);
      return { success: false, error: error2.message };
    }
  });
    ...
    try {
      ...
      setImmediate(async () => {
        try {
          ...
          console.error("[Mail IPC] Background mark as read error:", error2);
        }
      });
      return { success: true };
    } catch (error2) {
      console.error("[Mail IPC] Mark as read error:", error2);
      return { success: false, error: error2.message };
    }
  });
    ...
    try {
      ...
      setImmediate(async () => {
        try {
          ...
          console.error("[Mail IPC] Background delete error:", error2);
        }
      });
      return { success: true };
    } catch (error2) {
      console.error("[Mail IPC] Delete email error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:openAttachment", async (event, { attachmentPath }) => {
    try {
      ...
      console.error("[Mail IPC] Open attachment error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:startIdle", async (event, { accountId, folder }) => {
    try {
      ...
      const onNewMail = () => {
        ...
        allWindows.forEach((window2) => {
          if (window2 && !window2.isDestroyed()) {
            window2.webContents.send("mail:newMail", { accountId, folder, fresh: true });
          }
        });
      };
      ...
      console.error("[Mail IPC] Start IDLE error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:stopIdle", async (event, { accountId }) => {
    try {
      ...
      console.error("[Mail IPC] Stop IDLE error:", error2);
      return { success: false, error: error2.message };
    }
  });
    ...
    try {
      ...
      const transformedData = {
        ...
        domainReputation: {
          ...
          dnsConfig: {
            spf: ((_a = reputation.domain.spfRecord) == null ? void 0 : _a.status) === "good",
            dkim: ((_b = reputation.domain.dkimRecord) == null ? void 0 : _b.status) === "good",
            dmarc: ((_c = reputation.domain.dmarcRecord) == null ? void 0 : _c.status) === "good",
            mx: ((_d = reputation.domain.mxRecords) == null ? void 0 : _d.status) === "good",
            ssl: ((_e = reputation.domain.sslCertificate) == null ? void 0 : _e.status) === "good"
          },
          blacklists: ((_f = reputation.domain.blacklistStatus) == null ? void 0 : _f.map((bl) => ({
            name: bl.name,
        ...
        ipReputation: {
          ...
          reverseDns: ((_g = reputation.ip.reverseDNS) == null ? void 0 : _g.status) === "good",
          blacklists: ((_h = reputation.ip.blacklistStatus) == null ? void 0 : _h.map((bl) => ({
            name: bl.name,
            listed: bl.status === "bad"
          }))) || []
        },
      ...
      console.error("[Mail IPC] Reputation check error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:getSignatures", async (event, { accountId }) => {
    try {
      ...
      console.error("[Mail IPC] Get signatures error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:getDefaultSignature", async (event, { accountId }) => {
    try {
      ...
      console.error("[Mail IPC] Get default signature error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:addSignature", async (event, { accountId, name, htmlContent, isDefault }) => {
    try {
      const signatureId = crypto$1.randomUUID();
      addSignature({
        id: signatureId,
        accountId,
        name,
      ...
      console.error("[Mail IPC] Add signature error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:updateSignature", async (event, { signatureId, name, htmlContent, isDefault }) => {
    try {
      updateSignature(signatureId, name, htmlContent, isDefault);
      return { success: true };
    } catch (error2) {
      console.error("[Mail IPC] Update signature error:", error2);
      return { success: false, error: error2.message };
    }
  });
  ipcMain.handle("mail:deleteSignature", async (event, { signatureId }) => {
    try {
      ...
      console.error("[Mail IPC] Delete signature error:", error2);
      return { success: false, error: error2.message };
    }
  });
...
function setupAutoUpdater() {
  ...
  main$1.autoUpdater.on("error", (error2) => {
    console.error("[AutoUpdater] Error:", error2);
    sendUpdateStatus("error", { message: error2.message });
  });
  setInterval(() => {
...
function encryptForTransport(data, sessionKey) {
...
function initWebSocketServer() {
  try {
    ...
    wss.on("connection", (ws) => {
      ...
      pairedClients.set(ws, { paired: false });
      ws.on("message", async (data) => {
        try {
          ...
          if (message.type === "pair") {
            ...
            if (message.code === appId) {
              ...
              ws.send(JSON.stringify({
                ...
                sessionKey
                // Send session key to extension for encrypted comms
              }));
          ...
          console.error("[WebSocket] Error parsing message:", error2);
        }
      });
      ws.on("close", () => {
      ...
      ws.on("error", (error2) => {
        console.error("[WebSocket] Client error:", error2);
        extensionClients.delete(ws);
        pairedClients.delete(ws);
        wsRateLimits.delete(ws);
      });
    ...
    console.error("[WebSocket] Failed to start server:", error2);
  }
}
  ...
  try {
    ...
    if (clientInfo && clientInfo.sessionKey) {
      ...
      ws.send(JSON.stringify({
        ...
        encrypted: true
        // Flag to indicate credentials are encrypted
      }));
    } else {
      ws.send(JSON.stringify({
        type: "credentials-response",
    ...
    console.error("[WebSocket] Error handling credential request:", error2);
    ws.send(JSON.stringify({
      type: "credentials-response",
      success: false,
...
function getAllUserEmails() {
  ...
  try {
    ...
    console.error("[PasswordSync] Error getting emails:", error2);
    return [];
  }
}
async function pollForPendingPasswordChanges() {
  ...
  try {
    ...
    console.error("[PasswordSync] Poll error:", error2);
  }
}
async function updateCredentialPassword(email, newPassword, source) {
  if (!activeSession || !activeSession.masterPassword) {
  ...
  try {
    ...
    console.error("[PasswordSync] Update error:", error2);
    return false;
  }
}
async function confirmSyncWithAPI(eventIds) {
  try {
    ...
    console.error("[PasswordSync] Confirm error:", error2);
  }
}
...
function sendToExtension(data) {
  ...
  extensionClients.forEach((client) => {
    ...
    if (client.readyState === WebSocket.OPEN && clientInfo && clientInfo.paired) {
      if (clientInfo.sessionKey && data.type === "credentials" && data.username !== void 0 && data.password !== void 0) {
        const encryptedData = {
          ...data,
...
const createWindow = async () => {
  mainWindow = new BrowserWindow({
    ...
    minHeight: 600,
    frame: false,
    icon: path$n.join(process.env.VITE_PUBLIC, "favicon.ico"),
    webPreferences: {
      nodeIntegration: false,
      contextIsolation: true,
      preload: path$n.join(__dirname, "preload.mjs"),
      webSecurity: !isDev
    },
    show: false
  });
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    await mainWindow.loadFile(path$n.join(RENDERER_DIST, "index.html"));
  }
...
const createStickyNoteWindow = (noteId, noteData = {}) => {
  ...
  const noteUrl = VITE_DEV_SERVER_URL ? `${VITE_DEV_SERVER_URL}#/sticky-note/${noteId}` : `file://${path$n.join(RENDERER_DIST, "index.html")}#/sticky-note/${noteId}`;
  console.log("Loading sticky note URL:", noteUrl);
  if (VITE_DEV_SERVER_URL) {
    noteWindow.loadURL(noteUrl);
  } else {
    noteWindow.loadFile(path$n.join(RENDERER_DIST, "index.html"), {
...
const createMailWindow = () => {
  console.log("Creating Es Mail window...");
  if (mailWindow && !mailWindow.isDestroyed()) {
    mailWindow.focus();
    return mailWindow;
  }
  const preloadPath = path$n.join(__dirname, "preload.mjs");
  console.log("[Mail Window] Preload script path:", preloadPath);
  console.log("[Mail Window] Preload script exists:", fs__default.existsSync(preloadPath));
  ...
  mailWindow.once("ready-to-show", () => {
    mailWindow == null ? void 0 : mailWindow.show();
  });
  if (VITE_DEV_SERVER_URL) {
    mailWindow.loadURL(`${VITE_DEV_SERVER_URL}mail-window.html`);
  } else {
    mailWindow.loadFile(path$n.join(RENDERER_DIST, "mail-window.html"));
  }
  if (isDev) {
...
ipcMain.on("open-external-url", async (event, url) => {
  ...
  try {
    ...
    console.error("[Main] Failed to open external URL:", error2);
  }
});
...
ipcMain.on("sticky-note-toggle-always-on-top", (event) => {
  const window2 = BrowserWindow.fromWebContents(event.sender);
  if (window2) {
    const isAlwaysOnTop = window2.isAlwaysOnTop();
    window2.setAlwaysOnTop(!isAlwaysOnTop);
...
ipcMain.on("close-sticky-note-window", (event, noteId) => {
  if (stickyNoteWindows.has(noteId)) {
    const window2 = stickyNoteWindows.get(noteId);
    if (window2 && !window2.isDestroyed()) {
      window2.close();
    }
    stickyNoteWindows.delete(noteId);
...
ipcMain.handle("check-for-updates", async () => {
  try {
    ...
    console.error("[AutoUpdater] Check failed:", error2);
    return { success: false, error: error2.message };
  }
});
...
ipcMain.handle("open-in-browser", async (event, url, browser2, credentialId) => {
  console.log(`Opening ${url} in ${browser2}`);
  try {
    const browserPaths = {
      "chrome": [
    ...
    const paths = browserPaths[browser2];
    ...
    if (!browserPath) {
      console.error(`${browser2} not found in any of the expected locations`);
      return { success: false, error: `${browser2} not found` };
    }
      ...
      try {
        ...
        if (!activeSession.masterPassword) {
          ...
          return new Promise((resolve, reject) => {
            ...
            browserProcess.on("error", (error2) => {
              console.error(`Failed to open ${browser2}:`, error2);
              reject({ success: false, error: `Failed to launch ${browser2}` });
            });
            browserProcess.unref();
            console.log(`Successfully opened ${url} in ${browser2}`);
        ...
        console.error("[Auto-Fill] ❌ Error processing credentials:", error2);
      }
    ...
    return new Promise((resolve, reject) => {
      const browserProcess = spawn(browserPath, [url], {
        ...
        stdio: "ignore",
        shell: false
        // Important: don't use shell to prevent injection
      });
      browserProcess.on("error", (error2) => {
        console.error(`Failed to open ${browser2}:`, error2);
        reject({ success: false, error: `Failed to launch ${browser2}` });
    ...
    console.error("Browser launch error:", error2);
...
function getDeviceFingerprint() {
  const deviceInfo = `${os$2.hostname()}-${os$2.platform()}-${os$2.arch()}-${os$2.userInfo().username}`;
...
ipcMain.handle("auth:signup", async (event, { username, password }) => {
  try {
    ...
    return { success: false, error: error2.message || "Signup failed" };
  }
});
ipcMain.handle("auth:login", async (event, { username, password }) => {
  try {
    ...
    return { success: false, error: error2.message || "Login failed" };
  }
});
ipcMain.handle("auth:verify", async (event) => {
  ...
  if (persistedUser) {
    ...
    try {
      ...
      console.error("Error verifying persisted user:", error2);
      store.delete("user");
      activeSession = null;
...
ipcMain.handle("auth:verifyMasterPassword", async (event, { masterPassword }) => {
  try {
    ...
    const db2 = getDb$1();
    const user = db2.prepare("SELECT password_hash, salt, master_verify_token FROM users WHERE id = ?").get(activeSession.userId);
    if (!user) {
    ...
    if (!user.master_verify_token) {
      try {
        ...
        const verifyToken = encryptPassword("VAULT_VERIFY_TOKEN_" + Date.now(), encryptionKey);
        db2.prepare("UPDATE users SET master_verify_token = ? WHERE id = ?").run(verifyToken, activeSession.userId);
        console.log("[Auth] Generated verification token for legacy user");
      } catch (error2) {
        console.error("[Auth] Failed to generate verification token:", error2);
      }
    }
    ...
    console.error("[Auth] Verification error:", error2);
    return { success: false, error: error2.message || "Verification failed" };
  }
});
ipcMain.handle("credentials:fetch", async (event, { masterPassword, categoryId, search }) => {
  try {
    ...
    console.error("Fetch credentials error:", error2);
    return { success: false, error: error2.message || "Failed to fetch credentials" };
  }
});
ipcMain.handle("credentials:create", async (event, data) => {
  try {
    ...
    console.error("Create credential error:", error2);
    return { success: false, error: error2.message || "Failed to create credential" };
  }
});
ipcMain.handle("credentials:update", async (event, data) => {
  try {
    ...
    console.error("Update credential error:", error2);
    return { success: false, error: error2.message || "Failed to update credential" };
  }
});
ipcMain.handle("credentials:delete", async (event, { id }) => {
  try {
    ...
    console.error("Delete credential error:", error2);
    return { success: false, error: error2.message || "Failed to delete credential" };
  }
});
ipcMain.handle("categories:fetch", async (event) => {
  try {
    ...
    console.error("Fetch categories error:", error2);
    return { success: false, error: error2.message || "Failed to fetch categories" };
  }
});
ipcMain.handle("categories:create", async (event, { name, color }) => {
  try {
    ...
    console.error("Create category error:", error2);
    return { success: false, error: error2.message || "Failed to create category" };
  }
});
ipcMain.handle("categories:update", async (event, { id, name, color }) => {
  try {
    ...
    console.error("Update category error:", error2);
    return { success: false, error: error2.message || "Failed to update category" };
  }
});
ipcMain.handle("categories:delete", async (event, { id }) => {
  try {
    ...
    console.error("Delete category error:", error2);
    return { success: false, error: error2.message || "Failed to delete category" };
  }
});
ipcMain.handle("apikeys:fetch", async (event) => {
  try {
    ...
    const db2 = getDb$1();
    const apiKeys = db2.prepare(`
      SELECT id, provider, api_key, created_at, updated_at
      FROM api_keys WHERE user_id = ? ORDER BY provider
    `).all(activeSession.userId);
    const maskedKeys = apiKeys.map((key) => ({
      ...key,
      api_key_masked: key.api_key.length > 3 ? "•".repeat(key.api_key.length - 3) + key.api_key.slice(-3) : "•••",
      api_key: void 0
      // Don't send actual key
    }));
    return { success: true, apiKeys: maskedKeys };
  } catch (error2) {
    console.error("Fetch API keys error:", error2);
    return { success: false, error: error2.message || "Failed to fetch API keys" };
  }
});
ipcMain.handle("apikeys:save", async (event, { provider, apiKey }) => {
  try {
    ...
    console.error("Save API key error:", error2);
    return { success: false, error: error2.message || "Failed to save API key" };
  }
});
ipcMain.handle("apikeys:delete", async (event, { provider }) => {
  try {
    ...
    console.error("Delete API key error:", error2);
    return { success: false, error: error2.message || "Failed to delete API key" };
  }
});
function getApiKeyForProvider(provider) {
  ...
  return (result == null ? void 0 : result.api_key) || null;
}
ipcMain.handle("notes:fetch", async (event, { masterPassword } = {}) => {
  try {
    ...
    console.error("Fetch notes error:", error2);
    return { success: false, error: error2.message || "Failed to fetch notes" };
  }
});
ipcMain.handle("notes:create", async (event, { title, content, color, masterPassword }) => {
  try {
    ...
    console.error("Create note error:", error2);
    return { success: false, error: error2.message || "Failed to create note" };
  }
});
ipcMain.handle("notes:update", async (event, { id, title, content, color, position_x, position_y, width, height, masterPassword }) => {
  try {
    ...
    console.error("Update note error:", error2);
    return { success: false, error: error2.message || "Failed to update note" };
  }
});
ipcMain.handle("notes:delete", async (event, { id }) => {
  try {
    ...
    console.error("Delete note error:", error2);
    return { success: false, error: error2.message || "Failed to delete note" };
  }
});
const BACKUP_RETENTION_DAYS = 7;
function getBackupDir() {
  return path$n.join(app.getPath("userData"), "backups");
}
...
function performBackup() {
  try {
    ensureBackupDir();
    const dbPath = path$n.join(app.getPath("userData"), "database.sqlite");
    const backupDir = getBackupDir();
    const backupFileName = getBackupFileName();
    const backupPath = path$n.join(backupDir, backupFileName);
    ...
    fs__default.copyFileSync(dbPath, backupPath);
    console.log("[Backup] ✅ Backup created:", backupFileName);
    cleanupOldBackups();
    return { success: true, path: backupPath };
  } catch (error2) {
    console.error("[Backup] ❌ Backup failed:", error2);
    return { success: false, error: error2.message };
  }
}
function cleanupOldBackups() {
  try {
    const backupDir = getBackupDir();
    if (!fs__default.existsSync(backupDir)) return;
    const files = fs__default.readdirSync(backupDir);
    const now = Date.now();
    const maxAge = BACKUP_RETENTION_DAYS * 24 * 60 * 60 * 1e3;
    for (const file2 of files) {
      if (!file2.startsWith("database-backup-")) continue;
      const filePath = path$n.join(backupDir, file2);
      const stats = fs__default.statSync(filePath);
      const age = now - stats.mtimeMs;
      if (age > maxAge) {
        fs__default.unlinkSync(filePath);
        console.log("[Backup] 🗑️ Deleted old backup:", file2);
      }
    }
  } catch (error2) {
    console.error("[Backup] Error cleaning up old backups:", error2);
  }
}
function listBackups() {
  try {
    ...
    for (const file2 of files) {
      ...
      const filePath = path$n.join(backupDir, file2);
      const stats = fs__default.statSync(filePath);
      const dateMatch = file2.match(/database-backup-(\d{4}-\d{2}-\d{2})/);
    ...
    console.error("[Backup] Error listing backups:", error2);
    return [];
  }
}
function restoreBackup(backupName) {
  try {
    const backupDir = getBackupDir();
    const backupPath = path$n.join(backupDir, backupName);
    const dbPath = path$n.join(app.getPath("userData"), "database.sqlite");
    if (!fs__default.existsSync(backupPath)) {
      return { success: false, error: "Backup file not found" };
    }
    const currentBackupName = `database-pre-restore-${Date.now()}.sqlite`;
    const currentBackupPath = path$n.join(backupDir, currentBackupName);
    if (fs__default.existsSync(dbPath)) {
      fs__default.copyFileSync(dbPath, currentBackupPath);
      console.log("[Backup] Created pre-restore backup:", currentBackupName);
    }
    fs__default.copyFileSync(backupPath, dbPath);
    console.log("[Backup] ✅ Restored backup:", backupName);
    return { success: true };
  } catch (error2) {
    console.error("[Backup] ❌ Restore failed:", error2);
    return { success: false, error: error2.message };
  }
}
...
Respond in this exact JSON format:
{
  ...
  "improvements": ["<specific text changes or additions to make>"]
    ...
    console.error("[OpenAI] Error:", error2);
    return { success: false, error: error2.message || "Failed to analyze email" };
  }
});
...
Respond in this exact JSON format:
{
  ...
  "body": "<reformatted email body with proper formatting>"
    ...
    console.error("[OpenAI] Error:", error2);
    return { success: false, error: error2.message || "Failed to reformat email" };
  }
});
...
Respond in this exact JSON format:
{
  ...
  "improvements": [
    ...
    console.error("[OpenAI] Error:", error2);
    return { success: false, error: error2.message || "Failed to validate email" };
  }
});
...
Respond in this exact JSON format:
{
  ...
  "body": "<complete professional email body with proper formatting, greeting, and signature placeholder>"
    ...
    console.error("[OpenAI] Error:", error2);
    return { success: false, error: error2.message || "Failed to generate email" };
  }
});
app.whenReady().then(async () => {
  try {
    ...
    initMailIPC(
      ...
      // Session getter
      getDb$1
      // Database getter (already a function)
    );
    ...
    console.error("Failed to start application:", error2);
    app.quit();
  }
});
...
app.on("before-quit", () => {
  ...
  mailWindow = null;
  stickyNoteWindows.forEach((window2) => {
